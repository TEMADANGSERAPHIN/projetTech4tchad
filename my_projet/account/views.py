...
def homesubit(request):
  template = loader.get_template('homesubit.html')
  return HttpResponse(template.render())
...
def recu_listAgs(request):
	agence = PayeAgence.objects.all().order_by('nom')
	return render(request, 'payement/recu_listAgs.html', {'agence': agence,})
...
############### list non modifiable
def recu_lists(request):
	payetel = PayeTel.objects.all().order_by('telephone')
	return render(request, 'payement/recu_lists.html', {'payetel': payetel,})
#############################################
...
def recu_listBans(request):
	banque = PayeBan.objects.all().order_by('banque')
	return render(request, 'payement/recu_listBans.html', {'banque': banque,})
...
def show_banque(request, banque_id):
    banque = PayeBan.objects.get(pk=banque_id)
    return render(request, 'payement/show_banque.html', {'banque': banque,})
...
def employes_list(request):
	reservation = Reservation.objects.all().order_by('nom')
	return render(request, 'gestionnaire/employes_list.html', {'reservation': reservation,})
...
def show_employe(request, reservation_id):
    ...
    return render(request, 'gestionnaire/show_employe.html', {'reservation': reservation,})
...
def search_employe(request):
  # sourcery skip: remove-unnecessary-else, use-named-expression
    if request.method == "GET":
        ...
        if query:
            mutiple_q = Q (Q(email=query) | Q(nom=query))
        reservation = Reservation.objects.filter(mutiple_q)
        if reservation:
...
# 	superviseurs = Superviseur.objects.all().order_by('nom')
# 	return render(request, 'gestionnaire/superviseur_list.html', {'superviseurs': superviseurs,})

# def add_superviseur(request):
#   # sourcery skip: assign-if-exp, boolean-if-exp-identity, introduce-default-else, move-assign-in-block, remove-unnecessary-cast
#       submitted = False
#       if request.method == "POST":
#             form = SuperviseurForm(request.POST, request.FILES)
#             if form.is_valid():
#                   form.save()
#             return HttpResponseRedirect('/add_superviseur?submitted=True')
#       else:
#             form = SuperviseurForm
#       if 'submitted' in request.GET:
#             submitted=True
#       return render(request, 'gestionnaire/add_superviseur.html', {
#         'form': form,
#         'submitted': submitted,
#         })

# def show_superviseur(request, superviseur_id):
#     superviseurs = Superviseur.objects.get(pk=superviseur_id)
#     return render(request, 'gestionnaire/show_superviseur.html', {'superviseurs': superviseurs,})
      
# def delete_superviseur(request, superviseur_id):
#     superviseur = Superviseur.objects.get(pk=superviseur_id)
#     superviseur.delete()
#     return redirect('superviseur_list')


# def update_superviseur(request, superviseur_id):
#     superviseur = Superviseur.objects.get(pk=superviseur_id)
#     form = SuperviseurForm(request.POST or None, instance=superviseur)
#     if form.is_valid():
#         form.save()
#         return redirect('superviseur_list')
#     return render(request, 'gestionnaire/update_superviseur.html', {
#         'superviseur': superviseur,
#         'form': form,
#     })  

# def search_superviseur(request):
#   # sourcery skip: remove-unnecessary-else, use-named-expression
#     if request.method == "GET":
#         query = request.GET.get('query')
#         if query:
#             mutiple_q = Q (Q(matricule=query) | Q(nom=query))
#         superviseurs = Superviseur.objects.filter(mutiple_q)
#         if superviseurs:
#             return render(request, 'gestionnaire/superviseur_list.html', {
...
# 	directeur = Directeur.objects.all().order_by('nom')
# 	return render(request, 'gestionnaire/directeurs_list.html', {'directeur': directeur,})
